  ...
  useEffect(() => {
    ...
    const handleMessageReceived = ({
      ...
      if (isCurrentChat) {
        setMessages((prevMessages) => [
          ...prevMessages,
          { senderId, receiverId, message, fileUrl, productUrl, status, timestamp },
        ]);
    ...
    const handleMessageStatusUpdated = ({ chatId, messageId, status, messages }) => {
      if (messages) {
        // Handle bulk status updates
        setMessages((prevMessages) => 
          prevMessages.map(msg => {
            const updatedMsg = messages.find(m => m.messageId === msg._id);
            if (updatedMsg) {
