...
import { BiCheckDouble, BiEdit, BiCheck } from "react-icons/bi";
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        notifyError("Failed to upload file: " + result.message);
      }
    } catch (error) {
      console.error("Error uploading file", error);
      notifyError("Error uploading file: " + error.message);
    }
  };
          ...
          performSearchOnChatList.map(
            ({ chatPartner, userDetail, unseenMessagesCount }, index) => (
              <div
                ...
                className={`chat-list-item ${chatPartner.peerId === selectedChatUserId
                    ? "chat-list-active"
                    : ""
                  }`}
              >
              ...
              <em className="chats-messages-time">
                ...
                {isCurrentUser && (
                  <span className="message-status">
                    {msg.seen ? (
                      <BiCheckDouble size={16} style={{ color: '#34B7F1' }} />
                    ) : msg.delivered ? (
                      <BiCheckDouble size={16} style={{ color: '#FFFFFF' }} />
                    ) : (
                      <BiCheck size={16} style={{ color: '#FFFFFF' }} />
                    )}
                  </span>
                )}
                {isCurrentUser && (
    ...
    return (
      <div className="selected-chat-user-info-wrapper" ref={userInfoRef}>
        ...
        <div className="selected-user-call-wrapper">
          <button
            className={`selected-user-video-call-btn vCall ${isUserBlocked ? "disabled" : ""
              }`}
            ...
            <div className="gchat-h-icon-wr">
              <div className="gchat-add-person-wrapper">
                ...
                <div
                  ...
                  className={`gchat-add-new-contact-wrapper ${isAddNewContactVisible ? "visible" : "hidden"
                    }`}
                >
          ...
          <div className="chat-box-wrapper">
            <div className="chat-box-header">
              ...
              <div className="chat-box-opt">
                <div
                  className={`chat-box-opt-icon ${isUserBlocked ? "disabled" : ""
                    }`}
                  onClick={
                    !isUserBlocked
